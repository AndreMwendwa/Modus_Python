...
def demande_gratuite(n, itern):
    dbfile = open(f'{dir_dataTemp}gratuite', 'wb')
    pkl.dump(0, dbfile)
    dbfile.close()
    if PPM == 1:
        print("Calcul de la demande pour l'année de calage  \n")
    ...
    if PPS == 1:
        distribution.distribution(n, 'PPS')
        choix_modal.choix_modal(n, 'PPS', itern)
    # On fait ça deux fois pour avoir toutes les matrices qui entre dans la discussion de la gratuité.
    dbfile = open(f'{dir_dataTemp}gratuite', 'wb')
    pkl.dump(1, dbfile)
    dbfile.close()
    if PPM == 1:
...
def bouclage_func_gratuite(idBcl, MaxIter):
    if idBcl == 0:
        itern = 1
        print("Calcul de la demande pour l'année de scénario  \n")
        dir_iter = os.path.join(out_bcl, 'Iter1')
        ...
        if PPS == 1:
            distribution.distribution('scen', 'PPS')
            choix_modal.choix_modal('scen', 'PPS', itern)
        # On fait ça deux fois pour avoir toutes les matrices qui entre dans la discussion de la grauité.
        dbfile = open(f'{dir_dataTemp}gratuite', 'wb')
        pkl.dump(1, dbfile)
        dbfile.close()
        if PPM == 1:
        ...
        while done_affect < PPM + PCJ + PPS:
            ...
            # print(done_affect)
            time.sleep(
                60)  # Tant que la fonction d'affectation de VISUM défini dans Quatre_Etapes.affect n'a pas fini
            # de tourner, on s'arrête ici et on regarde tous les 60 secs pour voir s'il a terminé ou non.
        RMSE_PPM, RMSE_PCJ, RMSE_PPS = 0, 0, 0
        if PPM == 1:
            RMSE_PPM = bouclage.RMSE('PPM', 1)
        ...
        while (RMSE_PPM > cConv_M or RMSE_PCJ > cConv_C or RMSE_PPS > cConv_S) and itern <= MaxIter:
            ...
            if idBcl == 1:
                ...
                if PPS == 1:
                    RMSE_PPS = bouclage.RMSE('PPS', itern)
                bouclage.data_update(cParTpsBcl, 'scen')
            # # On ne va pas permettre idBcl == 2/3 pour la gratuité.
            # elif idBcl == 2 or idBcl == 3:
            #     if PPM == 1:
            #         utility.utilite('scen', 'PPM')
            #         choix_modal.choix_modal('scen', 'PPM', itern)
        ...
        bouclage.data_update(0, 'scen')
        bouclage.boucle(0, itern, dir_iter)
    # TODO: The VISUM network created in the final iteration (the one that comes after convergence has been attained,
    #  for whatever reason, is not being saved in the right place.
...
if __name__ == '__main__':
    # demande('scen', 1)
    # copy_files() # Copie des fichiers dans les dossiers Data, Quatre_etapes, Traitement dans le dossier des résultats
    print("Bienvenue à Modus_Python. \n Vous avez choisi d'utiliser le modèle en mode ")
    if idBcl == 0:
        print("sans bouclage")
    else:
    ...
    logging.info('Début')
    if gratuite_on:
        demande_gratuite('actuel', 0)
        bouclage_func_gratuite(idBcl, cNbBcl)
    else:
        demande('actuel', 0)
        bouclage_func(idBcl, cNbBcl)
    indicateurs_func()
